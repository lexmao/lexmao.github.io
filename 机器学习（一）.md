...
一组特征值，就是一 个向量；所以，这里实质上是测试2个向量的距离，这可以用欧氏距离公式搞定：<br>
	...
	b是机器学习的已经知道的特征矩阵(m*n)：b=[ [b1,b2,b3,b4....bn]
     ...
     D1(b1,a) =  √ (b1-a1)^2 +(b2-a2)^2+(b3-a3)^2 ...(bn-an)^2
     D2(c1,a) =  √ (c1-a1)^2 +(c2-a2)^2+(c3-a3)^2....(cn-an)^2
     D3(d1,a) =  √ (d1-a1)^2 +(d2-a2)^2+(d3-a3)^2....(dn-an)^2
     
     ...
     Dn(X1,a)=...
     
     最后获得一个m*1 大小的矩阵 D = [ [D1]
 								   [D2]
                                  [D3]
                                  [D4]
                                  ....
...
[demo代码](https://github.com/lexmao/blog_source/tree/master/KNN)
